...
def output(mybuffer,r1_write,r2_write):
    ...
    for i in range(length):
        ...
        if not isinstance(seqhead1,str):  
            seqhead1 = seqhead1 + b'\n'
            seq1_o = seq1_o.encode() + b'\n'
            qualhead1 = qualhead1  + b'\n'
            qual1_o = qual1_o.encode() + b'\n'
            seqhead2 = seqhead2 + b'\n'
            seq2_o = seq2_o.encode() + b'\n'
            qualhead2 = qualhead2  + b'\n'
...
def atac_trim(seq1_o,qual1_o,seq2_o,qual2_o):
    ...
    seq2 = seq2.decode()
    seq1 = seq1.decode()
    qual1 = qual1_o.decode()
    qual2 = qual2_o.decode()
...
def trim(R1_rds,R2_rds,r1_write,r2_write,adaptor_list,adaptor_list_rc,args):
    ...
    args_list = buffer2()
    while 1:
        if mybuffer_index <= 100:

            R1_line = R1_rds.readline().rstrip()
            R2_line = R2_rds.readline().rstrip()
            if not R1_line:                  
                break

            else:
                args_list.R1_lines.append(R1_line)
                args_list.R2_lines.append(R2_line)
                mybuffer_index += 1
        else:
            rlt=subread(args_list)
...
def main(kwargs):
    ...
    trim(R1_rds,R2_rds,r1_write,r2_write,adaptor_list,adaptor_list_rc,args)
